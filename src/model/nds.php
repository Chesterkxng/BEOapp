...
class NDSRepository
{
    ...
    public function addNDS(string $reference, string $object, string $edition_date, string $url): bool
    {
        $statement = $this->connection->getConnection()->prepare(
            "INSERT INTO `nds`(`REFERENCE`, `OBJECT`, `EDITION_DATE`, `URL`) 
            VALUES (?,?,?,?)"
        );
    ...
    public function updateNDS(string $reference, string $object, string $edition_date, int $nds_id): bool
    {
        $statement = $this->connection->getConnection()->prepare(
            "UPDATE `nds` 
            SET 
        ...
        $statement->execute([$reference, $object, $edition_date, $nds_id]);
