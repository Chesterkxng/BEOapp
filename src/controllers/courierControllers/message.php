...
class MSG
{
    ...
    public function addMSG(array $input)
    {
        ...
        if ($input !== null) {
            ...
            $edition_date = null;
            if (
                !empty($input['reference']) && !empty($input['object'])
            ...
            if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $location)) {
                ...
                $MSGRepository = new MessageRepository();
                $MSGRepository->connection = new DatabaseConnection();
                $success = $MSGRepository->addMessage($type, strtoupper($reference), strtoupper($object), $edition_date, $url);
                if ($success == 0) {
    ...
    public function updateMSG(array $input, int $msg_id)
    {
        ...
        if ($input !== null) {
            ...
            $success = $MSGRepository->updateMessage($type, strtoupper($reference), strtoupper($object), $edition_date, $msg_id);
            if ($success == 0) {
                echo '<script type="text/javascript">
                            updateErrorAlert()
                        </script>';
