...
class BE
{
    ...
    public function addBE(array $input)
    {
        ...
        if ($input !== null) {
            ...
            if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $location)) {
                ...
                $BERepository = new BordereauRepository();
                $BERepository->connection = new DatabaseConnection();
                $success = $BERepository->addBordereau(strtoupper($reference), strtoupper($object), $edition_date, $url);
                if ($success == 0) {
    ...
    public function updateBE(array $input, int $be_id)
    {
        ...
        if ($input !== null) {
            ...
            $success = $BERepository->updateBordereau(strtoupper($reference), strtoupper($object), $edition_date, $be_id);
            if ($success == 0) {
                echo '<script type="text/javascript">
                            updateErrorAlert()
                        </script>';
