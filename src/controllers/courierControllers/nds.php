...
class NDS
{
    ...
    public function addNDS(array $input)
    {
        ...
        if ($input !== null) {
            ...
            if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $location)) {
                ...
                $NDSRepository = new NDSRepository();
                $NDSRepository->connection = new DatabaseConnection();
                $success = $NDSRepository->addNDS(strtoupper($reference), strtoupper($object), $edition_date, $url);
                if ($success == 0) {
    ...
    public function updateNDS(array $input, int $nds_id)
    {
        ...
        if ($input !== null) {
            ...
            $success = $NDSRepository->updateNDS(strtoupper($reference), strtoupper($object), $edition_date, $nds_id);
            if ($success == 0) {
                echo '<script type="text/javascript">
                            updateErrorAlert()
                        </script>';
