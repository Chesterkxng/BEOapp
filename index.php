...
// stage IMPORT
require_once('src/controllers/stageControllers/stage.php');
...
use Application\Controllers\StageControllers\Stage\Stage;
...
try {
    ...
    switch ($action) {
        ...
        case 'uploadMO':
            ...
            // STAGE ROUTES
        case 'stagesList':
        case 'stageAddingForm':
        case 'addStage':
        case 'stageUpdatingForm':
        case 'updateStage':
        case 'deleteStagePopup':
        case 'deleteStage':
            if (checkAuthorization($_SESSION['profile_type'])) {
                switch ($action) {
                    case 'stagesList':
                        (new Stage())->StagesList();
                        break;
                    case 'stageAddingForm':
                        (new Stage())->stageAddingForm();
                        break;
                    case 'addStage':
                        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                            $input = $_POST;
                            (new Stage())->addStage($input);
                        }
                        break;
                    case 'stageUpdatingForm':
                        $stage_id = $_GET['stage_id'];
                        (new Stage())->stageUpdatingForm($stage_id);
                        break;
                    case 'updateStage':
                        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                            $stage_id = $_GET['stage_id'];
                            $input = $_POST;
                            (new Stage())->updateStage($input, $stage_id);
                        }
                        break;
                    case 'deleteStagePopup':
                        $stage_id = $_GET['stage_id'];
                        (new Stage())->sendDeletePopup($stage_id);
                        break;

                    case 'deleteStage':
                        $stage_id = $_GET['stage_id'];
                        (new Stage())->DeleteStage($stage_id);
                        break;
